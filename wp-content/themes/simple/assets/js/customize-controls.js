/* global simpleBgColors, simpleColor, jQuery, wp, _ */
/**
...
( function() {
	...
	wp.customize.bind( 'ready', function() {
		...
		wp.customize( 'accent_hue', function( value ) {
			value.bind( function( to ) {
				...
				Object.keys( simpleBgColors ).forEach( function( context ) {
					var backgroundColorValue;
					if ( simpleBgColors[ context ].color ) {
						backgroundColorValue = simpleBgColors[ context ].color;
					} else {
						backgroundColorValue = wp.customize( simpleBgColors[ context ].setting ).get();
					}
					simpleSetAccessibleColorsValue( context, backgroundColorValue, to );
				} );
			} );
		} );
		...
		Object.keys( simpleBgColors ).forEach( function( context ) {
			wp.customize( simpleBgColors[ context ].setting, function( value ) {
				value.bind( function( to ) {
					// Update the value for our accessible colors for this area.
					simpleSetAccessibleColorsValue( context, to, wp.customize( 'accent_hue' ).get(), to );
				} );
			} );
		} );

		// Show or hide retina_logo setting on the first load.
		simpleSetRetineLogoVisibility( !! wp.customize( 'custom_logo' )() );

		// Add a listener for custom_logo changes.
		wp.customize( 'custom_logo', function( value ) {
			value.bind( function( to ) {
				// Show or hide retina_logo setting on changing custom_logo.
				simpleSetRetineLogoVisibility( !! to );
			} );
		} );
	} );
	 ...
	 * @return {void}
	 */
	function simpleSetAccessibleColorsValue( context, backgroundColor, accentHue ) {
		var value, colors;
		...
		colors = simpleColor( backgroundColor, accentHue );
	 ...
	 * @return {void}
	 */
	function simpleSetRetineLogoVisibility( visible ) {
